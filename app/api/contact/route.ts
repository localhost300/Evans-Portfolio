...
export async function POST(request: Request) {
  try {
    ...
    if (!enquiry.name || !enquiry.subject || !enquiry.message ||
    ...
    await transporter.sendMail({
      ...
      html: `<h2>New website enquiry</h2><p><strong>Name:</strong> ${safe.name}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Phone:</strong> ${safe.phone || 'Not provided'}</p><p><strong>Location:</strong> ${safe.location}</p><p><strong>Subject:</strong> ${safe.subject}</p><p style="white-space:pre-wrap">${safe.message}</p>`,
    });
